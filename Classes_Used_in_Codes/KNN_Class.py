...
#metrics = ['discret_frechet', 'hausdorff', 'dtw', 'sspd', 'erp', 'edr', 'lcss', SoftDTW, 
#           fastdtw, dtw, d_Q_pi]
...
class KNN:
    def __init__(self, data1, data2, metric, gamma=None, eps_edr=None, eps_lcss=None, 
                 Q_size=None, Q=None, p=2, path=None, test_size=0.3, n_neighbors=5, num_trials=1000, 
                 pair=None):
        '''data1 = data[pair[0]]
           data2 = data[pair[1]]'''
        self.data1 = data1
    ...
    def calculate_dists_matrix(self):
        ...
        if str(self.metric) in ['discret_frechet', 'hausdorff', 'dtw', 'sspd', 'erp']: 
        ...
        if self.metric == dtw: # dtw = tslearn.metrics.dtw
            A = []
            for i in range(n-1):
                for j in range(i+1, n):
    ...
    def KNN_Classifier(self):
        ...
        if self.path:
            ...
            print("There is no pre-calculated distance matrix")
